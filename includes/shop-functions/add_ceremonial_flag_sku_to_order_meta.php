...
add_action('woocommerce_thankyou', 'add_ceremonial_flag_sku_to_order_meta', 10, 1);

function add_ceremonial_flag_sku_to_order_meta($order_id) {
    ...
    foreach ($order->get_items() as $item_id => $item) {
        ...
        if (has_term($terms_to_check, 'product_cat', $item->get_product_id())) {
			...
			$flag = preg_replace('/\s*\([^)]+\)$/', '', $meta_array['Flag']);
            $size = $meta_array['Size'];
            $gold_fringe = $meta_array['Add Gold Fringe'];
			
 			//echo "<pre>Info:" . print_r($meta_array, true) . "</pre>";
            
            // Query the custom DB table for a matching SKU
            global $wpdb;
            $table_name = 'ceremonial_flag_skus';
			$query = $wpdb->prepare("
			SELECT `Part_Code` FROM $table_name
			WHERE Flag = %s AND Size = %s AND Add_Gold_Fringe = %s", $flag, $size, $gold_fringe);
