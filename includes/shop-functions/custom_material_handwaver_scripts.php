...
function custom_material_handwaver_scripts() {
	...
	?>
        ...
        <script>
			...
			window.addEventListener('load', () => {
				...
				//
				//
				//Update unit price to reflect discount on quantity change
				//
				//
				...
				const customQuanityField = document.querySelector('#field_31_39.gfield--type-number input');
				...
				customQuanityField.addEventListener('change', (e) => {
					...
					// Get the text content of the element and remove any leading/trailing whitespaces
					const priceText = unitPrice.textContent.trim();
					// Remove any commas from the price text
					const cleanPriceText = priceText.replace(/,/g, '');
					const calculatedPrice = parseFloat(cleanPriceText.slice(1));
...
function custom_material_handwavers() {
	$pageID = get_queried_object_id();
		if($pageID == 524957) {
			// execute code to run on this specific page
			add_action( 'wp_footer', 'custom_material_handwaver_scripts' );
		}
}
add_action( 'wp', 'custom_material_handwavers' );