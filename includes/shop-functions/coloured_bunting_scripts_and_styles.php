...
add_action( 'wp_footer', 'coloured_bunting_scripts_and_styles' );

function coloured_bunting_scripts_and_styles() {
    ...
    if($pageID == 541656) : ?>
        ...
        <script>
			...
			if(document.body.classList.contains('postid-541656')) {
				...
				window.addEventListener('load', () => {
					...
					setTimeout(() => { 
						...
						const customQuanityField = document.querySelector('#field_41_44.gfield--type-number input');
						...
						customQuanityField.addEventListener('change', (e) => {
							...
							for (const range in productDiscounts) {
								...
								if(quantity < parseInt(min)) {
									discount = null;
									quantityMessageEl.textContent = `Add ${parseInt(min) - quantity} more to your basket to qualify for a 2.5% discount to this item.`;
									break;
								}
							...
							if(!discount) {
								...
								unitPriceEl.innerHTML = `&pound;${formattedUnitPrice} <span class="per-flag-text"> per length</span>`;
							...
							const observer = new MutationObserver(() => {
								...
								const splitPrice = unitPrice.innerHTML.split('').slice(1).join('');
								const currentUnitPrice = parseFloat(splitPrice);
				...
				//Update options if user selects from the pre-determind dropdown
				
				const preSelectedOptions = document.querySelector('#field_41_49 select');
				
				if(preSelectedOptions) {
					
					const setColourOptions = (e) => {
						
						const selectedOptions = e.currentTarget.value;
						
						//Split this into array
						const optionsArray = selectedOptions.split("|")[0].split(", ");
						
						//Fetch last item of the array and set this to be material
						const selectedMaterial = selectedOptions.split("|")[1];
						const materialOptions = document.querySelectorAll('#field_41_1 input');
						
						materialOptions.forEach(material => {
							
							if(material.nextElementSibling.textContent.toLowerCase().trim() === selectedMaterial.toLowerCase().trim()) {
								material.click();
							}
							
						})
						
						//Set Number of alternating colours
						const alternatingColourInput = document.querySelector('#input_41_4');
						alternatingColourInput.value = optionsArray.length;
						var event = new Event('change', { bubbles: true });
						// Dispatch the event
						alternatingColourInput.dispatchEvent(event);
						
						//Fetch colour options
						const colourOptions = document.querySelectorAll('.gfield--type-select[data-conditional-logic="visible"]');
						
						//Make sure both arrays have the same length
						if(colourOptions.length !== optionsArray.length) {
							throw new Error("The number of options and the number of select fields do not match!");
						}

						//Loop through selected options array and then loop through colour options and set the selected value
						for(let i = 0; i < optionsArray.length; i++) {
							const selectedOption = optionsArray[i];
							const option = colourOptions[i];
							const optionInput = option.querySelector('select');
							const options = Array.from(optionInput.options);
							const optionValue = options.find(option => option.value.toLowerCase() === selectedOption.toLowerCase().trim());
					...
					preSelectedOptions.addEventListener('change', setColourOptions)
